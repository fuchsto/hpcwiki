...
measurement pi_parallel(long long niter) {
  ...
  /* Atomic accumulate:
   * for i : 0..niter
   *   atomic { pi_4th_acc += <value in series at index> }
   */
  #pragma omp parallel for shared(pi_4th_acc)
  for (i = 0; i < niter; i++) {
    double tmp;
    if (i % 2 == 0) {
      tmp = 1.0 / (double)(2 * i + 1);
    } else {
      tmp = -1.0 / (double)(2 * i + 1);
    }
    #pragma omp atomic
...
int main(int argc, char * argv[]) {
  ...
  printf("Calculation of Pi (shared atomic accumulate)\n");
